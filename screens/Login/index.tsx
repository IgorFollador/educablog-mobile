...
const Login = () => {
  ...
  const writeUserDataToStorage = async (token: string, name: string) => {
  ...
  const handleLogin = async () => {
    ...
    await api.post("auth", body).then(response => {
      if (response.status == 200) {
        ...
        writeUserDataToStorage(response.data.token, response.data.userName);
  ...
  return (
    <View style={style.container}>
      ...
      <TextInput
        id="email"
        placeholderTextColor={theme.color_brand_dark}
        style={style.input}
        placeholder="E-mail"
